...
Vendored from [here](https://github.com/axboe/liburing/releases/tag/liburing-2.10). We only need the headers, but we can't get them through the build system, as the upstream repo contains files with an extension of `.3` which isn't allowed on Windows, breaking cross compilation.
